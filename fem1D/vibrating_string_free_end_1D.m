...
% Constants
density = 1E2; %1;
Youngs_modulus = 1E2; %100;
gravitational_acceleration = 0; 
load = 0.3;
height = 1; %height/length

% Mesh properties
number_elements = 40; % number of elements
...
velocity_initial = zeros(number_elements +1,1) %0.1*sin((pi/(2*height))*mesh);
...
displacement_exact = zeros(number_elements + 1, number_time_steps);
velocity_exact = zeros(number_elements +1 ,number_time_steps);

for n = 1:number_elements+1
[position_exact(n,:) displacement_exact(n,:) velocity_exact(n,:)]  = exact_solution_load(density,Youngs_modulus,load,gravitational_acceleration,height,mesh(n),t);
end
...
if anim_displ == 1
    for i = 1:length(time_interval)
        ...
        animation_displacement_vs_x(n*t_step, displacement_fem(:,n),...
            displacement_exact(:,n), mesh, 8E-3, -8E-3, height)
        pause(0.1);
    end
end
