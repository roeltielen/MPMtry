%% MPM_1D_Linear_Final (uses element_particle.m, particle_element.m, assemble_matrix, assemble_vector)
%  Roel Tielen
%  November 16th 2016
%  TU Delft
%  Based on the initial code of Lisa Wobbes. 

function[u_n, v_p, v_n, mass_p,u_p,E_kin,E_pot,E_grav,E_trac, F_diff, F_int_plot,F_ext_plot, stress_particle,indicator,strain_p] = MPM_1D_Linear_Final(density, E, g, load, H, n_e, h, n_ep,...
    pos_p_glob,  pos_p_loc, t_step, n_time_steps, total_time, u_p, v_p_int, stress_p, u_n,...
    v_n, both_ends_fixed, change_glob_pos,change_loc_pos, E_kin,E_pot,E_grav,E_trac,lagranian, volume_update,lumped, ULFEM,momentum,alpha,reset,deformation) 
...
elemets_nodes_original = zeros(n_e,2);
...
v_n = zeros(n_n,n_time_steps-1);
strain_p = zeros(n_p,n_time_steps-1);
v_p = zeros(n_p, n_time_steps-1);
v_p(:,1) = v_p_int; 
...
for s = 1:n_time_steps-1
        ...
        if num_particles_in_e(el) > 0 
            for p = element_particle(elements_particles(el,:))
                if ULFEM == 1
                    ...
                    if lumped == 1                       
                        M_e(1,1,el) = M_e(1,1,el) + h_temp*density_p(p)*N_vec(p,1);
                        M_e(2,2,el) = M_e(2,2,el) + h_temp*density_p(p)*N_vec(p,2);  
                    else
                        M_e(1,1,el) = M_e(1,1,el) + h_temp*density_p(p)*N_vec(p,1)*N_vec(p,1);
                        M_e(1,2,el) = M_e(1,2,el) + h_temp*density_p(p)*N_vec(p,1)*N_vec(p,2);
                        M_e(2,1,el) = M_e(2,1,el) + h_temp*density_p(p)*N_vec(p,2)*N_vec(p,1);
                        M_e(2,2,el) = M_e(2,2,el) + h_temp*density_p(p)*N_vec(p,2)*N_vec(p,2);
                    end
                    P_e(:,el)       = P_e(:,el) + h_temp*density_p(p)*(N_vec(p,:))'*v_p(p,s); 
                    F_grav_e(:,el)  = F_grav_e(:,el) + (N_vec(p,:))'*h_temp*density_p(p)*g;                  
                    F_int_e(:,el)   = F_int_e(:,el) + (B_temp)'*stress_p(p)*h_temp;    
                else    
                    if lumped == 1
    ...
    F_damp = - sign(v_n(:,s))*alpha.*abs(F_trac + F_grav - F_int);
   ...
   if reset == 1
    ...
    for p = 1:n_p
        if ULFEM == 1
           h_temp = elements_nodes(p,2) - elements_nodes(p,1);
           B_temp = [-1/h_temp, 1/h_temp];
    ...
    if ULFEM == 1
        if deformation == 1
            stress_p = stress_p + E*dstrain_p - stress_p.*dstrain_p;
        else
