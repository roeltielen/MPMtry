...
addpath('D:\wobbes\Documents\MATLAB\NEW\MPMtry\mpm1D')
%% Input
% Constants
density = 1;
Youngs_modulus = 100;
...
if shape == 1
    [displacement_mpm, velocity_mpm,velocity_mpm_nodes, M_lump, displacement_mpm_particles, E_kin,E_pot,E_grav,E_trac, F_diff,F_int_plot,F_ext_plot, stress_particle,indicator] = MPM_1D_Linear_Final(density,...
...
if velo_time == 1
    ...
    plot(t,velocity_mpm(particle_number,:),'k','LineWidth',2)
    hold on
...
if force_internal_plot == 1
    figure(5)
    plot(t,F_int_plot,'k','LineWidth',1.5)
    if pulse == 1
        hold on
    ...
    legend('F_{int}','Grid crossing direct', 'Grid crossing indirect',...
        'Location','Northwest')
